...
$ognoo =  post('ognoo', 8);
if (strlen($ognoo) == 5) {
    strlen($ognoo) < 5 ? '0' . $ognoo : $ognoo;
    $ognoo = date('Y') . '-' . str_replace('/', '-', $ognoo);
} else {
    $ognoo = '20' . str_replace('/', '-', $ognoo);
}
...
try {
    $success = _exec(
        ...
        [
            ...
            $avlaga_usuh, $avlaga_buurah, $ur_usuh, $ur_buurah, $orlogo, $zardal, $_SESSION['id'], $zuruuUtga
        ],
        $count

    );
} catch (Exception $e) {
    $_SESSION['errors'] = ["Systemiin aldaa garlaa. Ta daraa dahin oroldono uu" . ' : ' . $e->getMessage()];
} finally {
